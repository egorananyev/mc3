...
if(out || render){
    ...
    plotIndiv <- function(pss, subjStr='subj', xStr='targEcc', grpStr='targV', 
        ...
        p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
            ...
            labs(x=xlab, y=ylab, colour=collab) + ylim(c(-2,0)) + 
            guides(colour=guide_legend(keyheight=.3, default.unit='inch'))
        p <- themefy(p)
    }
}
...
## Loading the data
```{r}
allDataDir <- paste(db,'Projects/mc/data_mc3/v4 - 6vs',sep='')
...
```{r}
thresh <- ddply(ds, .(subj,dom,maskV,targV,targEcc), summarise, 
...
if(render){
    plotIndivFn <- function(pss){
        ss <- sumFn(pss)
        plot(plotAve(ss) + ggtitle(paste0('subj:', unique(pss$subj), ' ', 
                                              'maskV:', unique(pss$maskV))))
    }
    a <- dlply(thresh, .(subj,maskV), plotIndivFn)
}
```
...
```{r, fig.height=2, fig.width=3}
if(render || out){
    groupPlot <- function(pss){
        ss <- sumFn(pss)
        plot(themefy(plotAve(ss[ss$subj=='average',]) + ggtitle(paste0('maskV:',
                                                                       unique(pss$maskV)))))
    }
    p <- dlply(thresh, .(maskV), groupPlot)
    if(out){
