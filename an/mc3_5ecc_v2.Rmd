...
## Cross-size effects (no far target)
```{r}
## Regression-style analysis on binary vars:
dfnf <- dsb[dsb$targEcc_!='far',] # dfNoFar
dfnf$targEcc <- dfnf$targEcc + 1
pvalfn(lmer(thresh ~ dom + stairStart + maskV * targTon * targEcc * maskSz +
                 (1|subj), data=dfnf))
pvalfn(lmer(thresh ~ dom * stairStart * maskV * targTon * targEcc * maskSz +
                 (1|subj), data=dfnf))
```

## Only large mask (no small mask)
```{r}
dfns <- dsb[dsb$maskSz>0,] # dfNoFar
pvalfn(lmer(thresh ~ dom + stairStart + maskV * targTon * targEcc +
                 (1|subj), data=dfns))
pvalfn(lmer(thresh ~ dom * stairStart * maskV * targTon * targEcc +
                 (1|subj), data=dfns))
```
