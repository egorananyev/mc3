...
if(out || render){
    ...
    sumFn <- function(ss, subjStr='subj', xStr='targV', grpStr='targEcc',
                      xPlotStr='maskV', yPlotStr='monoStr'){
        sumSubj <- ddply(ss, c(subjStr, xStr, grpStr, xPlotStr, yPlotStr), summarise, 
        ...
        sumGrp <- ddply(sumSubj, c(xStr, grpStr, xPlotStr, yPlotStr), summarise,
    ...
    plotAve <- function(pss, subjStr='subj', xStr='targEcc', grpStr='targV', 
        ...
        p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
            ...
            facet_grid(reformulate(xPlotStr,yPlotStr)) +
    ...
    plotIndiv <- function(pss, xPlotStr='maskV', yPlotStr='monoStr', xStr='targEcc', 
                          grpStr='targV', xlab=xLab, ylab=yLab, collab=colLab, 
                          yStr='mn', seStr='se'){
        pss$yMin <- pss[,yStr] - pss[,seStr]
        pss$yMax <- pss[,yStr] + pss[,seStr]
        pss[,grpStr] <- factor(pss[,grpStr])
        ...
        p <- ggplot(pss, aes_string(x=xStr, y=yStr, colour=grpStr, group=grpStr,
            ...
            labs(x=xlab, y=ylab, colour=collab) + ylim(c(-2,1)) + 
            guides(colour=guide_legend(keyheight=.3, default.unit='inch'))
        p <- themefy(p)
    }
}
...
## Loading the data
```{r}
allDataDir <- paste(db,'Projects/mc/data_mc3/v5 - mono',sep='')
dataDirs <- dir(allDataDir)
dataDirs <- dataDirs[grep('mc3',dataDirs)]
colsOfInt <- c('participant', 'dom', 'mcBv', 'mono', 'targBv', 'targXoff2', 
               'stairStart', 'meanRev6')
df <- data.frame()
dfRevs <- data.frame()
dfIntn <- data.frame()
    ...
    if(qc){
        ...
        for(curStairFN in subjStairs){
            ...
            curInfoCols <- data.frame(subj=curDf$participant[1], dom=curDf$dom[1], 
                                      stairStart=curInfo[which(curInfo==" 'startContr':")+1], 
                                      mono=curInfo[which(curInfo==" 'mono':")+1], 
                                      maskV=curInfo[which(curInfo==" 'mcBv':")+1], 
                                      targV=curInfo[which(curInfo==" 'targBv':")+1], 
                                      targEcc=curInfo[which(curInfo==" 'targXoff2':")+1])
...
if(qc){
    ...
    dfIntn$targV <- round(as.numeric(levels(dfIntn$targV))[dfIntn$targV] * 60 / 35, 1)
    dfIntn$targV[dfIntn$targV<0.05] <- 0
    dfIntn$targStr <- 'dynamic'
    dfIntn$targStr[dfIntn$targV==0] <- 'static'
    dfIntn$monoStr <- 'monocular'
    dfIntn$monoStr[dfIntn$mono==0] <- 'interocular'
    head(dfIntn)
}
...
ds$targV <- round(ds$targV * 60 / 35, 1)
ds$targStr <- 'dynamic'
ds$targStr[ds$targV==0.01] <- 'static'
ds$monoStr <- 'monocular'
ds$monoStr[ds$mono==0] <- 'interocular'
head(ds)
```
...
```{r}
thresh <- ddply(ds, .(subj,dom,maskV,monoStr,targV,targEcc), summarise, 
...
if(qc){
    plotQc <- function(pss){
        plot(ggplot(pss, aes(x=trial, y=intn, colour=targEcc, linetype=stairStart)) +
            facet_wrap(monoStr*targStr ~ maskV, ncol=8) + geom_point() + geom_line() + 
                ylim(-2,1) + ggtitle(paste0('subj:', unique(pss$subj))))
    }
    a <- dlply(dfIntn, .(subj), plotQc) 
}
```
...
if(render){
    plotIndivFn <- function(pss){
        ss <- sumFn(pss)
        plot(plotIndiv(ss) + ggtitle(paste0('subj:', unique(pss$subj))))
    }
    a <- dlply(thresh, .(subj), plotIndivFn)
}
```
...
if(render || out){
    ss <- sumFn(thresh)
    plot(themefy(plotAve(ss[ss$subj=='average',])))
    if(out){
